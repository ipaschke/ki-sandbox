...
## Profile local: internal model server only

For strictly confidential work the leitfaden allows only local AI (section
2.3). The sandbox has a second profile for that: no cloud AI endpoint is
reachable, and both harnesses talk to the internal inference server.

### Selecting it

- Per project: `echo local > ~/.config/sbx/projects/<p>/profile`. The
  project then always runs local; nothing can switch it back per call.
- Per call: `sbx-claude --local`, `sbx-codex --local`, `sbx-shell --local`.
- Profile and endpoint are part of the container identity: switching or
  changing the endpoint recreates the container (running processes end).

### Endpoint

`~/.config/sbx/local-model.env` (template: `local-model.env.example`) with
`SBX_LOCAL_BASE_URL` (http(s)://host[:port], no path), `SBX_LOCAL_MODEL`, an
optional `SBX_LOCAL_API_KEY` and an optional `SBX_LOCAL_CONTEXT_TOKENS`
(context window; Claude Code does not know local models and assumes 200k
otherwise). The server must offer the Anthropic Messages API
(`/v1/messages`, for Claude Code) and the OpenAI Responses API (`/v1`, for
Codex); llama.cpp's `llama-server`, Ollama and LiteLLM do. Without the file,
`--local` is refused. Both harnesses warn that the model is not in their
catalog; that is expected and harmless.

Verified 2026-09-16 against a `llama-server` (Qwen3.8-27B) on the LAN: firewall
opens only that host, `claude -p` and `codex exec` are answered by the local
model, api.anthropic.com is unreachable from the container.

### What differs from the cloud profile

| | cloud | local |
|---|---|---|
| Firewall | GitHub, npm, PyPI, nodesource, Ubuntu + Anthropic, OpenAI, claude.ai, telemetry | GitHub, npm, PyPI, nodesource, Ubuntu + the model host only |
...
`~/.local/state/sbx/projects/<p>/etc-sbx/`) carries `profile`, `allow`,
`local-model.env` (host, URL, model; never the key) and `codex/config.toml`.
The firewall refuses to start if `/etc/sbx` exists but is writable. The API
key reaches the container only as environment (`ANTHROPIC_AUTH_TOKEN`,
`SBX_LOCAL_API_KEY`), so it is visible to the agent: use a key that is valid
for the internal server and nothing else. Codex's shell environment policy
hides both variables from commands it runs.

### Checks after the first start

    sbx-shell --local
    curl -sS -o /dev/null -w '%{http_code}\n' https://api.anthropic.com/   # must fail
    curl -sS -o /dev/null -w '%{http_code}\n' "$ANTHROPIC_BASE_URL/"        # server answers
    claude -p 'Reply with exactly: ok'                                      # answered by the local model
    codex exec 'Reply with exactly: ok'                                     # Responses API on the server
